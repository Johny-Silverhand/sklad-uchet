...
Склад Учёт · Victimok Labs

© 2026 Victimok Labs. Все права защищены.

Это локальное десктопное приложение (Go + SQLite). Интерфейс открывается в окне Edge/Chrome (--app=). Данные хранятся только на вашем ПК.
...
function view() {
  ...
  if (id === "welcome") {
    pane.innerHTML = `
      ...
      <h1>Установка «Склад Учёт»</h1>
      <p class="lead">Локальный учёт запчастей, устройств и комплектующих. Работает на вашем ПК: SQLite, без облака. UI — в окне Edge/Chrome.</p>
      <div class="meta">
        <div><small>Издатель</small><b>Victimok Labs</b></div>
        <div><small>Версия</small><b>1.1.0 · 2026</b></div>
      </div>`;
  ...
  if (id === "options") {
    pane.innerHTML = `
      ...
      <div class="checks">
        <label class="check"><input type="checkbox" checked disabled /><span><b>Ядро «Склад Учёт»</b><i>Исполняемый модуль, локальная БД, подпись Victimok Labs</i></span></label>
        <label class="check"><input type="checkbox" id="sm" ${state.startMenu ? "checked" : ""} /><span><b>Меню «Пуск»</b><i>Victimok Labs → Склад Учёт</i></span></label>
